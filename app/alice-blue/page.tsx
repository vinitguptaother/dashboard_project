...
export default function AliceBluePage() {
  ...
  const fetchSingleQuote = async (exchange: string, symbol: string) => {
    ...
    try {
      const response = await fetch(`/api/alice-blue-ltp?exchange=${exchange}&symbol=${symbol}&totp=${totpCode}`);
  ...
  const fetchBatchQuotes = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/alice-blue-ltp?batch=true', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Alice Blue Live Prices</h1>
          <p className="text-gray-600">Real-time stock prices using Alice Blue API with TOTP authentication</p>
        </div>
        ...
        <div className="mb-6 bg-blue-50 border border-blue-200 rounded-lg">
          <div className="p-6">
            <h3 className="font-medium text-blue-900 mb-2">Alice Blue API Status</h3>
            <div className="text-sm text-blue-800 space-y-1">
              <div><code>AB_USER_ID</code> - Your Alice Blue user ID (configured ✓)</div>
              <div><code>AB_PASSWORD</code> - Your Alice Blue login password (configured ✓)</div>
              <div><code>AB_API_SECRET</code> - Your API secret from developer console (configured ✓)</div>
            </div>
            <p className="text-xs text-blue-600 mt-2">
              TOTP codes are entered manually above. Testing with updated API endpoints.
            </p>
          </div>
        </div>

        {/* API Status Warning */}
        <div className="mb-6 bg-yellow-50 border border-yellow-200 rounded-lg">
          <div className="p-6">
            <h3 className="font-medium text-yellow-800 mb-2">🔍 Alice Blue API Debug Status</h3>
            <div className="text-sm text-yellow-700 space-y-2">
              <p><strong>Current Status:</strong> API endpoint responds with 200 OK but returns empty JSON `{}`</p>
              <p><strong>Possible Issues:</strong></p>
              <ul className="list-disc list-inside ml-4 space-y-1">
                <li>Invalid credentials (username/password/API secret)</li>
                <li>TOTP code format or timing issue</li>
                <li>Missing required parameters or wrong parameter names</li>
                <li>API may require account activation for programmatic access</li>
              </ul>
              <p><strong>Next Steps:</strong></p>
              <ul className="list-disc list-inside ml-4 space-y-1">
                <li>Verify Alice Blue account has API access enabled</li>
                <li>Check if API secret is from the correct developer console</li>
                <li>Try with a fresh TOTP code from your authenticator app</li>
                <li>Contact Alice Blue support for updated API documentation</li>
              </ul>
            </div>
          </div>
        </div>
